...
Cinema was born to be a scratchpad application. A kitchen sink forked from the [Quick Start][electron-quick-start] app, where I could test code and ideas for another existing Electron project without messing the original source code. It eventually took its own road as a template application, a blueprint that provides common features and lets you concentrate on your application's specific code.
...
Cinema's logger is a simple `console.*` redirector. During development all the output is written to the terminal. When the application is packaged the output is redirected to a file:
...
Cinema comes pre-packaged and configured for [Electron Builder][electron-builder] with a **two** `package.json` files structure. Build settings can be customized in development `package.json`, you can find more details in the [wiki][electron-builder-wiki].
...
Cinema comes pre-packaged with other useful tools like `pre-comit`, [Devtron][devtron], and scripts for running tests and code coverage with [Spectron][spectron], [Mocha][mocha]/[Chai][chai] and [Istanbul][istanbul].
...
~~~ console
PS C:\src\Cinema> $env:CSC_LINK="https://url.to.my/certificate.p12"
PS C:\src\Cinema> $env:CSC_KEY_PASSWORD="CertificatePassword"
PS C:\src\Cinema> npm run build:win -- --sign
~~~
...
Cinema is released under the MIT License. See the bundled LICENSE file for details.
